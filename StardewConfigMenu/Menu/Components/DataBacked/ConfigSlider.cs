...
namespace StardewConfigMenu.Components.DataBacked {
	...
	internal class ConfigSlider: SliderComponent {
		...
		internal ConfigSlider(IRange option, int x, int y) : base(option.Label, option.Min, option.Max, option.StepSize, option.Value, option.ShowValue, x, y, option.Enabled) {
			ModData = option;
		}
	}
}
