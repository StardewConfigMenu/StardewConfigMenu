...
namespace StardewConfigMenu
{
    ...
    internal class DropDownComponent: OptionComponent
    {
        ...
        // Constructors
        //
        public DropDownComponent(string label, int width) //: base(label, x, y, (int)Game1.smallFont.MeasureString("Stardew Configuration Menu Framework     ").X + Game1.pixelZoom * 12, 11 * Game1.pixelZoom, whichOption)
        {
            AddListeners();
            this.label = label;
        ...
        public override void draw(SpriteBatch b, int x, int y)
        {
            ...
            // Draw Label
            b.DrawString(Game1.dialogueFont, this.label, new Vector2((float)(x + this.bounds.X + this.bounds.Width + Game1.pixelZoom), (float)(y + this.bounds.Y + ((this.bounds.Height - labelSize.Y) / 2))), Game1.textColor);
            ...
            if (this.IsActiveComponent())
            {
                ...
                {
                    if (i == this.hoveredChoice && dropDownBounds.Contains((int) MousePositionRelativeToComponent().X, (int)MousePositionRelativeToComponent().Y))
                    {
                        b.Draw(Game1.staminaRect, new Rectangle(x + this.dropDownBounds.X, y + this.dropDownBounds.Y + i * this.bounds.Height, this.dropDownBounds.Width, this.bounds.Height), new Rectangle?(new Rectangle(0, 0, 1, 1)), Color.Wheat, 0f, Vector2.Zero, SpriteEffects.None, 0.975f);
                    }
                    b.DrawString(Game1.smallFont, this.dropDownDisplayOptions[i], new Vector2((float)(x + this.dropDownBounds.X + Game1.pixelZoom), (float)(y + this.dropDownBounds.Y + Game1.pixelZoom * 2 + this.bounds.Height * i)), Game1.textColor * scale, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.98f);
                }
                b.Draw(Game1.mouseCursors, new Vector2((float)(x + this.bounds.X + this.bounds.Width - Game1.pixelZoom * 12), (float)(y + this.bounds.Y)), new Rectangle?(OptionsDropDown.dropDownButtonSource), Color.Wheat * scale, 0f, Vector2.Zero, (float)Game1.pixelZoom, SpriteEffects.None, 0.981f);
            }
            else
            {
                IClickableMenu.drawTextureBox(b, Game1.mouseCursors, OptionsDropDown.dropDownBGSource, x + this.bounds.X, y + this.bounds.Y, this.bounds.Width - Game1.pixelZoom * 12, this.bounds.Height, Color.White * scale, (float)Game1.pixelZoom, false);
                if (this.IsAvailableForSelection())
                {
                    b.DrawString(Game1.smallFont, (this.selectedOption >= this.dropDownDisplayOptions.Count || this.selectedOption < 0) ? string.Empty : this.dropDownDisplayOptions[this.selectedOption], new Vector2((float)(x + this.bounds.X + Game1.pixelZoom), (float)(y + this.bounds.Y + Game1.pixelZoom * 2)), Game1.textColor * scale, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.88f);
                }
                b.Draw(Game1.mouseCursors, new Vector2((float)(x + this.bounds.X + this.bounds.Width - Game1.pixelZoom * 12), (float)(y + this.bounds.Y)), new Rectangle?(OptionsDropDown.dropDownButtonSource), Color.White * scale, 0f, Vector2.Zero, (float)Game1.pixelZoom, SpriteEffects.None, 0.88f);
            }
        }
        ...
        public void receiveLeftClick(int x, int y)
        {
            ...
            if (!this.disabled && this.bounds.Contains(x - (int) location.X, y - (int) location.Y))
            {
        ...
        {
            if (!this.disabled && this.IsActiveComponent() && this.dropDownBounds.Contains(x - (int) location.X, y - (int) location.Y))
            {
                this.dropDownBounds.Y = Math.Min(this.dropDownBounds.Y, Game1.viewport.Height - this.dropDownBounds.Height);
                this.hoveredChoice = (int)Math.Max(Math.Min((float)(y - (int)location.Y - this.dropDownBounds.Y) / (float)this.bounds.Height, (float)(this.dropDownOptions.Count - 1)), 0f);
            }
        }

        public void leftClickReleased(int x, int y)
        {
            if (this.dropDownBounds.Contains(x - (int)location.X, y - (int)location.Y))
            {
